...
def deepnnwithrelu(images):  
  ...
  with tf.name_scope('dropout'):
    ...
    h_fc1_drop = tf.nn.dropout(hidden6, keep_prob)
...
def appstart(stri):
  ...
  if(stri=="relu"):
      y_conv, keep_prob = deepnnwithrelu(x)
  elif(stri=="elu"):
      y_conv, keep_prob = deepnnwithelu(x)
  else:
      y_conv, keep_prob = deepnnwithsigmoid(x)
...
axes.set_ylabel('Test Improvements')
plt.plot()
