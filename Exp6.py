...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 30 03:17:41 2017
...
Batch_Size = 100
LEARNING_RATE = 0.01
hiddenlayer_units1 =16
hiddenlayer_units2 =32
hiddenlayer_units3 =64
hiddenlayer_units4 =32
expno = "6"

def deepnnwithrelu(images):  
  with tf.name_scope('hiddenlayer1'):
    weights = tf.Variable(
        tf.truncated_normal([MNIST_IMAGE_PIXELS, hiddenlayer_units1],
                            stddev=1.0 / math.sqrt(float(MNIST_IMAGE_PIXELS))),
        name='weights')
    biases = tf.Variable(tf.zeros([hiddenlayer_units1]),
                         name='biases')
    hidden1 = tf.nn.relu(tf.matmul(images, weights) + biases)
  # Hidden 2
  with tf.name_scope('hiddenlayer2'):
    weights = tf.Variable(
        tf.truncated_normal([hiddenlayer_units1, hiddenlayer_units2],
                            stddev=1.0 / math.sqrt(float(hiddenlayer_units1))),
        name='weights')
    biases = tf.Variable(tf.zeros([hiddenlayer_units2]),
                         name='biases')
    hidden2 = tf.nn.relu(tf.matmul(hidden1, weights) + biases)
    
  with tf.name_scope('hiddenlayer3'):
    weights = tf.Variable(
        tf.truncated_normal([hiddenlayer_units2, hiddenlayer_units3],
                            stddev=1.0 / math.sqrt(float(hiddenlayer_units2))),
        name='weights')
    biases = tf.Variable(tf.zeros([hiddenlayer_units3]),
                         name='biases')
    hidden3 = tf.nn.relu(tf.matmul(hidden2, weights) + biases)
    
  with tf.name_scope('hiddenlayer4'):
    weights = tf.Variable(
        tf.truncated_normal([hiddenlayer_units3, hiddenlayer_units4],
                            stddev=1.0 / math.sqrt(float(hiddenlayer_units3))),
        name='weights')
    biases = tf.Variable(tf.zeros([hiddenlayer_units4]),
                         name='biases')
    hidden6 = tf.nn.relu(tf.matmul(hidden3, weights) + biases)

  with tf.name_scope('finallayer'):
    W_fc2 = weight_variable([hiddenlayer_units4, 10])
...
def deepnnwithsigmoid(images):  
  ...
  with tf.name_scope('hiddenlayer4'):
    ...
    biases = tf.Variable(tf.zeros([hiddenlayer_units4]),
                         name='biases')
    hidden6 = tf.nn.sigmoid(tf.matmul(hidden3, weights) + biases)
...
def deepnnwithelu(images):  
  ...
  with tf.name_scope('hiddenlayer4'):
    ...
    biases = tf.Variable(tf.zeros([hiddenlayer_units4]),
                         name='biases')
    hidden6 = tf.nn.elu(tf.matmul(hidden3, weights) + biases)
...
def weight_variable(shape):
  ...
  initial = tf.truncated_normal(shape, stddev=0.1/math.sqrt(float(hiddenlayer_units4)))
